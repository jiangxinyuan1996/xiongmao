...
export default class detail extends Component {
                ...
                <Swiper key={this.state.swiperlist.length}>
                    {
                        this.state.swiperlist.map(item =>
                            <div className="swiper-slide" key={item.seq}>
                                <img src={item.url} className={style.banner}></img>
                            </div>
                        )
                    }
                </Swiper>
                ...
                <div className={style.taobao}>
                    ...
                    <i></i>
                    {
                        this.state.photolist.map(item =>
                            <img src={item.photo ? item.photo.url : null} key={item.photo ? item.photo.id : item.text}></img>
                        )
                    }
                </div>
    ...
    componentDidMount() {
        ...
        axios.get(`http://www.xiongmaoyouxuan.com/api/detail?id=${this.props.match.params.myid}&normal=1&sa=`).then(res => {
